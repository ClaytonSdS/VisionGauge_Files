...
LOSS_CURVES = [
    ...
    ("train/cls_loss", "Perda de Classificação (Treino)", "Perda de Classificação"),
    ("train/dfl_loss", "Perda DFL (Treino)", "Perda DFL"),
    ("val/box_loss", "Perda de Caixa (Validação)", "Perda de Caixa"),
    ("val/cls_loss", "Perda de Classificação (Validação)", "Perda de Classificação"),
...
# =========================
# FIGURA 1 - LOSS (CORRIGIDA)
# =========================
fig1, axs = plt.subplots(2, 3, figsize=(14, 10))
fig1.subplots_adjust(hspace=0.35, wspace=0.3, top=0.9)

for i, (ax, (col, title, ylabel)) in enumerate(zip(axs.flat, LOSS_CURVES)):
...
fig1.legend(handles, labels, loc="upper center", ncol=3, fontsize=11, frameon=False)

fig1.savefig(
    "models\\detector_losses_2x3.png",
    dpi=300,
    bbox_inches="tight"
)
